/**
 * This script can be used to interact with the Loyalty contract, after deploying it.
 *
...
let zkAppLoyaltyress = zkAppKey.toPublicKey();
let zkApp = new Loyalty(zkAppLoyaltyress);

// compile the contract to create prover keys
console.log('compile the contract...');
await Loyalty.compile();

// call update() and send transaction
console.log('build transaction and create proof...');
let tx = await Mina.transaction({ sender: zkAppLoyaltyress, fee: 0.1e9 }, () => {
  zkApp.update();
});
await tx.prove();
